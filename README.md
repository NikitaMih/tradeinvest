...
На данной странице аккаунта выводится информация обо всех активах, имеющихся в портфеле. Круговая диаграмма изменяет пропорции в зависимости от объёма актива в портфеле.
...
На странице профиля выводится информация о пользователе. На карточке пользователя имеется возможность изменение данных и пароля.

- ### Help

<p align="center"><img src="screenshots/help.png" width="500"/></p>

На станице помощи имеется возможность отправки сообщения на сервер (http://localhost:3001/help). Объект на сервере сожержит: id, login, title, text.
...
3. Установить json-server, если ранее не был установлен;
   ...
   npm install -g json-server
    ```
4. Выполнить команду (после выполнения команды, на потре http://localhost:3000 развернется проект);
    ```
   npm start
    ```  
